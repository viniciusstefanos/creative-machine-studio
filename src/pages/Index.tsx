...
const Dashboard = () => {
  const { data: activations = [], isLoading: loadingAct } = useQueryWithToast<Activation[]>({
    queryKey: ["dashboard-activations"],
    queryFn: async () => {
    ...
    staleTime: 30_000,
    errorMessage: "Erro ao carregar ativações",
  });

  const { data: reviewQueue = [], isLoading: loadingReview } = useQueryWithToast<ReviewItem[]>({
    ...
    staleTime: 30_000,
    errorMessage: "Erro ao carregar fila de aprovação",
  });

  const { data: scheduledPosts = [], isLoading: loadingSched } = useQueryWithToast<any[]>({
  ...
  const loading = loadingAct || loadingReview || loadingSched;
      ...
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-3">
            ...
            {loadingAct ? <CardSkeleton count={3} /> : activations.length === 0 ? (
          ...
          <div className="space-y-3">
            ...
            {loadingReview ? <CardSkeleton count={3} /> : reviewQueue.length === 0 ? (
              <div className="card-base text-center py-6">
          ...
          <div className="space-y-3">
            ...
            {loadingSched ? <CardSkeleton count={3} /> : scheduledPosts.length === 0 ? (
              <div className="card-base text-center py-6">
