...
const SettingsTeam = () => {
  ...
  return (
    <AppLayout breadcrumbs={[{ label: "Configurações" }, { label: "Time" }]}>
      <h1 className="text-2xl font-bold mb-6" style={{ fontFamily: "'Syne', sans-serif", color: "hsl(var(--text-primary))" }}>
      ...
      {loading ? (
        <div className="text-sm mt-3" style={{ color: "hsl(var(--text-muted))" }}>Carregando...</div>
      ) : (
          ...
          {profiles.map((p) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium truncate" style={{ color: "hsl(var(--text-primary))", fontFamily: "'DM Sans'" }}>
                  {p.full_name || p.email || "Sem nome"}
                </p>
                <p className="text-[10px] truncate" style={{ fontFamily: "'JetBrains Mono', monospace", color: "hsl(var(--text-muted))" }}>
                  {p.email}
                </p>
              </div>
