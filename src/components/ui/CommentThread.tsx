    ...
    <div>
      ...
      <div className="mt-4 space-y-3">
        {loading ? (
          ...
          <div className="p-4 rounded-lg text-center" style={{ background: "var(--bg-surface1)", border: "1px solid var(--border-default)", borderRadius: 8 }}>
            <MessageSquare size={20} className="mx-auto mb-2" style={{ color: "var(--text-muted)" }} />
            <p className="text-xs" style={{ color: "var(--text-muted)", fontFamily: "'DM Sans'" }}>Nenhum comentário</p>
          </div>
        ) : (
          rootComments.map((comment) => (
            <div key={comment.id}>
              <div
                ...
                style={{
                  background: comment.resolved ? "var(--bg-base)" : "var(--bg-surface1)",
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <p className="text-xs flex-1" style={{ color: "var(--text-primary)", fontFamily: "'DM Sans'" }}>
                    {comment.body}
                  </p>
                  <button
                ...
                <div className="flex items-center justify-between mt-2">
                  <span className="text-[9px]" style={{ fontFamily: "'JetBrains Mono', monospace", color: "var(--text-muted)" }}>
                    {formatTime(comment.created_at)}
                  </span>
                  <button
                    onClick={() => setReplyTo(replyTo === comment.id ? null : comment.id)}
                    className="text-[10px]"
                    style={{ fontFamily: "'DM Sans'", color: "var(--accent)" }}
                  >
                    Responder
                  </button>
                </div>
              ...
              {getReplies(comment.id).map((reply) => (
                <div
                  ...
                  style={{ background: "var(--bg-surface2)", border: "1px solid var(--border-subtle)", borderRadius: 8 }}
                >
                  <p className="text-xs" style={{ color: "var(--text-primary)", fontFamily: "'DM Sans'" }}>{reply.body}</p>
                  <span className="text-[9px] mt-1 block" style={{ fontFamily: "'JetBrains Mono', monospace", color: "var(--text-muted)" }}>
              ...
              {replyTo === comment.id && (
                <div className="ml-6 mt-2 flex gap-2">
                  <input
                    ...
                    style={{ background: "var(--bg-base)", border: "1px solid var(--border-strong)", color: "var(--text-primary)", fontFamily: "'DM Sans'", borderRadius: 6 }}
