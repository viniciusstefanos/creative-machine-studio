        ...
        {showDropdown && (
          ...
          >
            <div className="px-4 py-3 flex items-center justify-between" style={{ borderBottom: "1px solid var(--border-subtle)" }}>
              <span className="text-xs font-medium" style={{ color: "var(--text-primary)", fontFamily: "'DM Sans'" }}>Notificações</span>
              <Link to="/notifications" onClick={() => setShowDropdown(false)} className="text-[10px]" style={{ color: "var(--accent)", fontFamily: "'JetBrains Mono', monospace" }}>
                Ver todas
              </Link>
            </div>
            {recent.length === 0 ? (
              <div className="px-4 py-6 text-center">
                <p className="text-xs" style={{ color: "var(--text-muted)", fontFamily: "'DM Sans'" }}>Nenhuma notificação</p>
              </div>
            ) : (
              <div>
                {recent.map((n) => (
                  ...
                  >
                    <p className="text-xs truncate" style={{ color: "var(--text-primary)", fontFamily: "'DM Sans'" }}>{n.message}</p>
                    <p className="text-[9px] mt-1" style={{ fontFamily: "'JetBrains Mono', monospace", color: "var(--text-muted)" }}>
